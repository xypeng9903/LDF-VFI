...
def main(args):
    ...
    for i in tqdm(
        ...
        # Create sampler.
        gt_reader = VideoFrameLazyReader(lq_path[i], dimension_order='NCHW')
        lq_reader = gt_reader[::args.temporal_sf]
        if args.resolution is not None:
            H, W = map(int, args.resolution.split('x'))
            lq_reader = lq_reader[..., :H, :W]
        ...
        if sp_rank == 0:
            ...
            # Reconstruct GT sequence from the lazy reader so we can save GT video and frames
            gt_files = gt_reader._files
            gt_seq = torch.stack([gt_reader[t] for t in range(len(gt_reader))])
            if args.resolution is not None:
                gt_seq = gt_seq[..., :H, :W]
            gt_uint8 = rearrange(gt_seq, 't c h w -> t h w c')

            pred_uint8 = pred_uint8[:len(gt_seq)]

            # Save videos.
            torchvision.io.write_video(os.path.join(output_dir, 'pred.mp4'), pred_uint8, fps=args.fps, **codec_kwargs)
            torchvision.io.write_video(os.path.join(output_dir, 'gt.mp4'), gt_uint8, fps=args.fps, **codec_kwargs)
            if args.save_lq:
            ...
            def save_image_pair(j_and_file):
                j, file = j_and_file
                try:
                    ...
                    pred_img = pred_uint8[j].detach().cpu().numpy()
                    gt_img = gt_uint8[j].detach().cpu().numpy()
                    Image.fromarray(pred_img).save(pred_save_dir / fname)
                    Image.fromarray(gt_img).save(gt_save_dir / fname)
                except Exception as e:
                    logger.error(f"Error saving frame {j}: {e}")
...
def parse_args():
    ...
    parser.add_argument("--task",                      type=str,              default="test-extreme",  choices=["test-extreme", "test-hard", "test-medium", "test-easy"])
